...
export default function KedisiplinanKamarPage() {
  const [dataList, setDataList] = useState<PenilaianKamar[]>([]);
  ...
  const loadData = () => {
    setLoading(true);
    fetch('/api/penilaian-asrama?tipe=KAMAR')
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleEdit = (p: PenilaianKamar) => {
    setEditingId(p.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedAsrama || !selectedNomorKamar) {
      alert('Asrama dan Nomor Kamar wajib dipilih!');
      return;
    }

    const payload = {
      tipe_penilaian: 'KAMAR',
    ...
    if (result.success) {
      alert(editingId ? 'Evaluasi Kamar Diperbarui!' : 'Evaluasi Kamar Disimpan!');
      handleCancel();
      loadData();
    } else {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Hapus data penilaian kamar ini?')) return;
    const res = await fetch('/api/penilaian-asrama', {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <main className="p-6 bg-[#f4f7fc] min-h-screen text-slate-700">
      <div className="bg-white p-5 rounded-3xl border border-slate-200/60 shadow-sm mb-6">
        <h1 className="text-2xl font-black text-[#1e293b] tracking-tight">Penilaian Kedisiplinan & Kerapihan Kamar</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-5 rounded-3xl border border-slate-200/60 shadow-sm h-fit">
          <h2 className="text-sm font-bold text-[#1e293b] mb-4">
            {editingId ? '✏️ Edit Evaluasi Kamar' : '📋 Input Evaluasi Kamar'}
          </h2>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-600 mb-1">NAMA ASRAMA / GEDUNG</label>
              <select
                required
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Kerapihan Tempat Tidur</label>
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Kerapihan Lemari</label>
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Kebersihan Lantai</label>
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Penataan Sandal/Sepatu</label>
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Kepatuhan Tatib Kamar</label>
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase">Inventaris Kamar</label>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-600 mb-1">CATATAN KAMAR</label>
              <textarea value={formData.catatan} onChange={(e) => setFormData({ ...formData, catatan: e.target.value })} placeholder="Catatan kebersihan kamar..." className="w-full border border-slate-200 p-2.5 rounded-xl text-xs outline-none h-16" />
            </div>
            ...
            <div className="flex gap-2 mt-2">
              ...
              <button type="submit" className={`${editingId ? 'w-1/2 bg-amber-600' : 'w-full bg-[#4f46e5]'} text-white text-xs py-2.5 rounded-xl font-bold shadow-md`}>
                {editingId ? 'Simpan Edit' : 'Simpan Evaluasi'}
              </button>
            </div>
          </form>
        </div>
          ...
          <table className="w-full text-left text-sm text-slate-700">
            <thead className="bg-slate-50 text-[11px] text-slate-400 uppercase font-bold border-b border-slate-100">
              <tr>
                ...
                <th className="px-4 py-3">ASRAMA & INFO KAMAR</th>
            ...
            <tbody className="divide-y divide-slate-100 text-xs">
              {loading ? (
                ...
                <tr><td colSpan={5} className="text-center py-6 text-slate-400">Belum ada riwayat penilaian kamar.</td></tr>
              ) : (
                dataList.map((p, idx) => (
                  <tr key={p.id} className="hover:bg-slate-50 align-top">
                    ...
                    <td className="px-4 py-3">
                      <div className="font-bold text-[#1e293b] text-sm">
                        Asrama {p.asrama} — Kamar {p.nama_kamar}
                      </div>
                      <div className="text-[10px] text-indigo-600 font-semibold mb-1">
                        📅 {p.tanggal} | 👨‍🏫 Musyrif: {p.nama_musyrif}
                      </div>
                      <div className="mt-1 p-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-600 italic">
                        <span className="font-bold not-italic text-[10px] text-slate-400 uppercase block mb-0.5">Catatan Evaluasi:</span>
